...
class ScenarioBuilderPage(QWidget, Ui_scenario_builder):
    ...
    def __init__(self,tabWidget, data_handler,power_system_data_page,planning_model_page, optimizer, results_viewer):
        ...
        # Connect radio buttons to slots if needed
        self.co2_emissions_button.toggled.connect(self.on_co2_emissions_button_toggled)
        self.co2_intensity_button.toggled.connect(self.on_co2_intensity_button_toggled)
    ...
    def on_co2_emissions_button_toggled(self, checked):
        if checked:
            self.data_handler.set_co2_policy(True)

    def on_co2_intensity_button_toggled(self, checked):
        if checked:
            self.data_handler.set_co2_intensity_policy(True)
    ...
    def on_view_scenario_button_clicked(self):
        ...
        scenario_info = {
            ...
            "CO2 Emissions Reductions:":self.data_handler.co2_policy,
            "CO2 Intensity Reductions":self.data_handler.co2_intensity_policy,
    ...
    def capital_cost_trajectory_help(self):
        ...
        self.popup_message.setText("Select the capital cost trajectory for energy storage technologies. The cast trajectories are low, mid, and high. These trajectories are defined in the input csv data.")
        #self.popup_message.setStandardButtons(QMessageBox.Ok)
        self.popup_message.exec_()
